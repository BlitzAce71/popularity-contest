...
DROP FUNCTION IF EXISTS public.force_advance_round(tournament_uuid UUID);

CREATE OR REPLACE FUNCTION public.force_advance_round(p_round_id UUID, p_tournament_id UUID)
RETURNS JSONB AS $$
DECLARE
...
-- Add comment
COMMENT ON FUNCTION public.force_advance_round(UUID, UUID) IS 'Force advance tournament round by declaring winners based on current vote leaders';