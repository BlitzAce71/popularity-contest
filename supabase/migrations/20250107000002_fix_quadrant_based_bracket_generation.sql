-- Fix bracket generation to use proper quadrant-based seeding
-- This replaces the sequential pairing with proper tournament seeding within each quadrant

-- Drop the old function first
DROP FUNCTION IF EXISTS public.generate_single_elimination_bracket(UUID);

-- Helper function to get proper seeding pairs for a bracket size
...
BEGIN
    ...
    quadrant_rounds := CEIL(LOG(2, quadrant_size));
