-- ================================================================
-- Migration: Fix User Authentication Issues
-- Description: Fix user profile creation, deletion, and authentication flow
-- Author: System
-- Date: 2025-01-16
-- ================================================================

-- ================================================================
-- 1. Create User Profile Creation Trigger
-- ================================================================
...
DECLARE
  ...
  insert_values TEXT;
BEGIN
  -- Check if can_create_tournaments column exists
  IF EXISTS (
    ...
    -- Include can_create_tournaments if it exists
  ...
  ELSE
    -- Exclude can_create_tournaments if it doesn't exist yet
    INSERT INTO public.users (
      id,
      email,
      username,
...
-- ================================================================
-- 2. Enhanced User Deletion Function
-- ================================================================
...
-- ================================================================
-- 3. User Profile Recovery Function
-- ================================================================
...
BEGIN
  ...
  LOOP
    -- Create missing profile (check if can_create_tournaments column exists)
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'users' 
      AND column_name = 'can_create_tournaments'
...
CREATE POLICY "Users can update own profile"
  ...
  WITH CHECK (
    auth.uid() = id AND
    -- Prevent users from changing admin status or tournament creator permissions
    is_admin = (SELECT is_admin FROM public.users WHERE id = auth.uid()) AND
    can_create_tournaments = (SELECT can_create_tournaments FROM public.users WHERE id = auth.uid())
  );
